...
export const profile: SessionRoute = async ({ req, p, method, s }) => {
  ...
  const provenance: Record<string, Provenance> = Object.fromEntries(
    keys.map((k) => [k, previous[k] === 'answered' ? 'answered' : 'entered']),
  );

  const updated = await profileRepository().save(s.id, s.version, {
    profile,
    confirmed: keys,
    provenance,
    checkpoint: 'PROFILE_CONFIRMED',
  });
  if (!updated) throw new HttpError(409, 'Profile changed in another request.');
  return json({
    profile,
    confirmed: keys,
    provenance,
...
export const answer: SessionRoute = async ({ req, p, method, s }) => {
  ...
  if (
    b.provenance !== undefined &&
    !['answered', 'inferred'].includes(b.provenance)
  )
    throw new HttpError(400, 'Invalid provenance.');
  const provenanceOf: Provenance =
    b.provenance === 'inferred' ? 'inferred' : 'answered';

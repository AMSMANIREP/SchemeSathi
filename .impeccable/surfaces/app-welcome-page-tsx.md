...
**FIRST VIEWPORT:** Two columns. Left, the display line "You describe your life. We find what you are owed.", the lede stating the mechanism, then a single Log in button, with the privacy line beneath. The button opens a conventional login dialog — email, password, Sign in — because the demo should look like the product it is standing in for. The dialog states plainly that it checks nothing, sends nothing and stores no password; a form that looks real and authenticates nothing should admit which one it is. Only a display name, taken from the part of the address before the @, is kept. Right, a real **pass** rendered oversized and rotated a degree off true — a document set down on a desk, not a card in a grid. The artifact the citizen leaves with is shown rather than described.
